...
def get_mcps_with_all_machines_ready(machine_config_pools_list):
    ...
    for mcp in machine_config_pools_list:
        ...
        if (
            ...
            and mcp_instance_status.readyMachineCount
            == mcp_instance_status.updatedMachineCount
        ):
...
def wait_for_mcp_updated_condition_true(
    ...
    consecutive_checks_for_mcp_condition(
        mcp_sampler=sampler, machine_config_pools_list=machine_config_pools_list
    )
...
def collect_mcp_data_on_update_timeout(
...
def wait_for_all_nodes_ready(nodes):
    ...
    try:
        ...
        if nodes_not_ready:
            LOGGER.error(
                f"Some nodes are not ready: {(node.name for node in nodes_not_ready)}"
            )
        raise
...
def wait_for_nodes_to_have_same_kubelet_version(nodes):
    ...
    try:
        ...
        if node_versions:
            LOGGER.error(
                f"The kubelet version is not the same for all nodes: {node_versions}"
            )
        raise
...
def update_subscription_source(subscription, subscription_source, subscription_channel):
    LOGGER.info(
