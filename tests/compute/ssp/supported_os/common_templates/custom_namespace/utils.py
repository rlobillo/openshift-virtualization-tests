...
def patch_template_labels(admin_client, hco_namespace, template):
    with ResourceEditor(
        patches={
            template: {
                "metadata": {
                    "labels": {TEMPLATE_PATCH_LABEL: TEMPLATE_PATCH_LABEL_VALUE}
                }
            }
        }
    ):
...
def wait_for_edited_label_reconciliation(template, timeout=TIMEOUT_3MIN):
    for sample in TimeoutSampler(
        wait_timeout=timeout,
        sleep=5,
        func=lambda: template.instance.metadata.labels[TEMPLATE_PATCH_LABEL]
        != TEMPLATE_PATCH_LABEL_VALUE,
    ):
        if sample:
            return
...
def verify_base_templates_exist_in_namespace(
    ...
    try:
        ...
        LOGGER.error(
            f"Templates not in namespace {namespace.name}: {missing_template_names}"
        )
        raise
...
def diskless_vm_from_template(client, name, namespace, base_template_labels):
    extracted_template_labels = extract_template_labels(
        template_labels=base_template_labels
    )
