...
def kubevirt_prometheus_service_monitor_list(admin_client):
    return list(
        cluster_resource(ServiceMonitor).get(
            dyn_client=admin_client,
            label_selector=f"app.kubernetes.io/managed-by={VIRT_OPERATOR}",
        )
    )
...
class TestPrometheusServiceMonitor:
    ...
    def test_kubevirt_service_monitor_namespace(
        self, kubevirt_service_monitor_namespace, hco_namespace
    ):
    ...
    def test_prometheus_service_monitor_in_our_namespace(
        ...
        for service_monitor_object in kubevirt_prometheus_service_monitor_list:
            assert (
                service_monitor_object.namespace == kubevirt_service_monitor_namespace
            )
