...
def assert_log_verbosity_level_in_virt_pods(virt_pods_list):
    failed_log_verbosity_level_pods = [
        ...
        if f"verbosity to {VIRT_LOG_VERBOSITY_LEVEL_6}" not in pod.log()
    ]

    assert (
        not failed_log_verbosity_level_pods
...
@pytest.fixture()
def node1_log_verbosity_level_6_config(worker_node1):
    return {
        "kubevirt": {"nodeVerbosity": {worker_node1.name: VIRT_LOG_VERBOSITY_LEVEL_6}}
    }


class TestLogVerbosityForVirtComponent:
    ...
    def test_component_log_verbosity(
        self, updated_log_verbosity_config, virt_component_pods
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                pytest.lazy_fixture("node1_log_verbosity_level_6_config"),
                marks=pytest.mark.polarion("CNV-8576"),
            ),
        ],
        indirect=True,
    )
    def test_node_log_verbosity(
        self,
        updated_log_verbosity_config,
        virt_component_pods_in_first_node,
    ):
        assert_log_verbosity_level_in_virt_pods(
