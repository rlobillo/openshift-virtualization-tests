...
def updated_common_template(
    ...
    assert (
        not modified_common_templates
    ), f"Following templates were not reverted back: {modified_common_templates}"
...
class TestModifyCommonTemplateSpec:
    ...
    def test_modified_common_template(
        self, updated_common_template, hyperconverged_status_templates_scope_function
    ):
        ...
        assert hco_template["status"].get(
            "modified"
    ...
    def test_common_template_modify_spec(
        ...
        for template_name in updated_template_names:
            ...
            if hco_error:
                errors.append(
                    f"Mismatch for template in hco.status: {template_name}: {''.join(hco_error)}.\n"
                )
            ...
            if ssp_error:
                errors.append(
                    f"Mismatch for template in ssp.spec: {template_name}: {''.join(ssp_error)}.\n"
                )
        ...
        [
            ...
            pytest.param(
                ...
                marks=(
                    ...
                    pytest.mark.dependency(
                        name="test_multiple_common_templates_config_disable"
                    ),
                ),
            ),
        ],
