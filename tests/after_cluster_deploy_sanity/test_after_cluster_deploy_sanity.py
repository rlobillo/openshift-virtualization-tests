import logging

import pytest
from ocp_resources.cdi import CDI
from ocp_resources.kubevirt import KubeVirt
from ocp_resources.machine_config_pool import MachineConfigPool
from ocp_resources.namespace import Namespace
from ocp_resources.network_addons_config import NetworkAddonsConfig
from ocp_resources.persistent_volume_claim import PersistentVolumeClaim
from ocp_utilities.infra import (
    assert_nodes_in_healthy_condition,
    assert_nodes_schedulable,
)

from utilities.constants import KUBELET_READY_CONDITION, TIMEOUT_10MIN
from utilities.hco import get_installed_hco_csv, wait_for_hco_conditions
from utilities.infra import storage_sanity_check, wait_for_pods_running
from utilities.operator import wait_for_cluster_operator_stabilize


# flake8: noqa: PID001
LOGGER = logging.getLogger(__name__)


@pytest.mark.cluster_health_check
def test_node_sanity(admin_client, nodes):
    assert_nodes_in_healthy_condition(
        nodes=nodes, healthy_node_condition_type=KUBELET_READY_CONDITION
    )
    assert_nodes_schedulable(nodes=nodes)


@pytest.mark.cluster_health_check
def test_pod_sanity(admin_client, hco_namespace):
    wait_for_pods_running(
        admin_client=admin_client,
        namespace=hco_namespace,
    )


@pytest.mark.cluster_health_check
def test_hyperconverged_sanity(admin_client, hco_namespace):
    wait_for_hco_conditions(
        admin_client=admin_client,
        hco_namespace=hco_namespace,
        list_dependent_crs_to_check=[CDI, NetworkAddonsConfig, KubeVirt],
    )


@pytest.mark.cluster_health_check
def test_storage_sanity(cluster_storage_classes_names):
    assert storage_sanity_check(
        cluster_storage_classes_names=cluster_storage_classes_names
    )


@pytest.mark.cluster_health_check
def test_pvc_health():
    not_bound = []
    for pvc in list(PersistentVolumeClaim.get()):
        pvc_status = pvc.instance.status.phase
        LOGGER.info(f"PVC {pvc.name} is in {pvc_status} state")
        if pvc_status != pvc.Status.BOUND:
            not_bound.append(f"{pvc.name} status: {pvc_status}")

    assert not not_bound, f"Following pvcs are not in bound state {not_bound}"


@pytest.mark.cluster_health_check
def test_namespace_health():
    namespace_error_status = []
    for namespace in list(Namespace.get()):
        namespace_status = namespace.instance.status.phase
        LOGGER.info(f"Namespace {namespace.name} is in {namespace_status} state")
        if namespace_status != Namespace.Status.ACTIVE:
            namespace_error_status.append(
                f"{namespace.name} status: {namespace_status}"
            )
    assert (
        not namespace_error_status
    ), f"Following namespaces are in not active status: {namespace_error_status}"


@pytest.mark.cluster_health_check
def test_cluster_operator_health(admin_client):
    wait_for_cluster_operator_stabilize(
        admin_client=admin_client, wait_timeout=TIMEOUT_10MIN
    )


@pytest.mark.cluster_health_check
def test_machine_config_pool_health():
    failed_mcps = []
    for mcp in list(MachineConfigPool.get()):
        mcp_instance = mcp.instance
        ready_count = mcp_instance.status.readyMachineCount
        machine_count = mcp_instance.status.machineCount
        if machine_count > 0:
            degraded_count = mcp_instance.status.degradedMachineCount
            LOGGER.info(
                f"MCP {mcp.name} has {ready_count}/{machine_count} nodes in ready state"
            )
            if int(ready_count) != int(machine_count):
                failed_mcps.append(
                    f"mcp: {mcp.name}, degraded count: {degraded_count}, ready count: {ready_count},"
                    f" machine count: {machine_count}"
                )

    assert not failed_mcps, f"MCP health check failed due to: {failed_mcps}"


@pytest.mark.cluster_health_check
def test_csv_health(admin_client, hco_namespace):
    csv = get_installed_hco_csv(admin_client=admin_client, hco_namespace=hco_namespace)
    csv_status = csv.instance.status.phase
    assert (
        csv_status == csv.Status.SUCCEEDED
    ), f"CSV {csv.name} is in state: {csv_status}"
