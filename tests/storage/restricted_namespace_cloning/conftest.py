...
def destination_namespace(admin_client):
    yield from create_ns(
        name="restricted-namespace-test-destination-namespace",
...
def restricted_namespace_service_account(destination_namespace):
    with cluster_resource(ServiceAccount)(
        name="vm-service-account", namespace=destination_namespace.name
    ) as sa:
        yield sa
...
def data_volume_clone_settings(
    destination_namespace, data_volume_multi_storage_scope_module
):
...
def perm_src_service_account(
    request, namespace, destination_namespace, restricted_namespace_service_account
):
...
def perm_destination_service_account(
    request, destination_namespace, restricted_namespace_service_account
):
    with set_permissions(
