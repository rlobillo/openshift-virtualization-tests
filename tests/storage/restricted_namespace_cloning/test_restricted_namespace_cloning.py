...
from tests.storage.utils import create_vm_from_dv, verify_vm_disk_image_permission
...
def test_user_permissions_positive(
    ...
    verify_snapshot_used_namespace_transfer(
        cdv=dv_destination_cloned_from_pvc, unprivileged_client=unprivileged_client
    )
    if requested_verify_image_permissions:
        with create_vm_from_dv(dv=dv_destination_cloned_from_pvc) as vm:
            if (
                storage_class_matrix__module__[storage_class_name_scope_module][
                    "volume_mode"
                ]
