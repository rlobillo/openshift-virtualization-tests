...
def dv_kwargs(
    ...
    return {
        ...
        "url": request.param.get(
            "url",
            f"{get_images_server_url()}{py_config['latest_fedora_os_dict']['image_path']}",
        ),
...
def test_hpp_not_specify_node_immediate(
    ...
    with create_dv(
        ...
        dv_name="cnv-3354",
        namespace=namespace.name,
        url=f"{get_images_server_url()}{Images.Windows.WIN16_IMG}",
...
def test_hpp_specify_node_immediate(
    ...
    with create_dv(
        ...
        dv_name="cnv-3228",
        namespace=namespace.name,
        url=f"{get_images_server_url()}{Images.Rhel.RHEL8_0_IMG}",
        content_type=DataVolume.ContentType.KUBEVIRT,
        size="35Gi",
        storage_class=[*storage_class_matrix_hpp_matrix__module__][0],
        hostpath_node=worker_node1.name,
