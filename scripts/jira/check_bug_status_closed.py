...
from scripts.utils import all_python_files, get_jira_connection_params, print_status


# Needs to be update based on the branch.
EXPECTED_TARGET_VERSIONS = ["4.12"]


def get_jira_connection():
    connection_params = get_jira_connection_params()
...
def get_all_jiras_from_file(file_content):
    ...
    - jira_id=<jira_id>  # call in is_jira_open
    - jira_id = <jira_id>  # when jira is constant
    - https://issues.redhat.com/browse/<jira_id>  # when jira is in a link in comments
    - pytest.mark.jira(<jira_id>)  # when jira is in a marker
...
def main():
    closed_statuses = get_jira_connection_params()["resolved_statuses"]
    closed_jiras = {}
